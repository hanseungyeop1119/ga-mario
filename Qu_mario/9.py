...
class MyApp(QWidget):
    ...
    def paintEvent(self, event):
       ...
       # full_screen_tiles = np.concatenate((full_screen_page1_tile, full_screen_page2_tile), axis=1).astype(np.int)
        painter.setPen(QPen(Qt.black, 1.0, Qt.SolidLine))
        for k in range(full_screen_page1_tile.shape[0]):
            for h in range(full_screen_page1_tile.shape[1]):
                if full_screen_page1_tile[k][h] != 0:
                    # 브러쉬 설정(채우기)
                   painter.setBrush(QBrush(Qt.green))
                   painter.drawRect(800+16*h, 16*k, 16, 16)
